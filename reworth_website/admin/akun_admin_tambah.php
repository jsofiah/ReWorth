...
// Fungsi untuk mendapatkan ID role berdasarkan nama role
function getRoleId($supabaseUrl, $supabaseKey, $roleName) {
    $url = $supabaseUrl . "/rest/v1/role?nama_role=eq." . urlencode($roleName) . "&select=id_role";
    $headers = [
        "apikey: $supabaseKey",
    ...
    return !empty($data) ? $data[0]['id_role'] : null;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $id_role = getRoleId($supabaseUrl, $supabaseKey, $roleName);
    ...
    $adminId = $authResult['user']['id'] ?? null;
    
    if (!$adminId) {
        echo json_encode(['success' => false, 'message' => 'Gagal mendapatkan ID admin']);
        exit;
    }
    ...
    $ch = curl_init($supabaseUrl . "/rest/v1/admin");
    ...
    if ($httpCode === 201 || $httpCode === 200 || $httpCode === 204) {
        ...
        $logData = [
            ...
            'aktivitas' => 'Menambahkan ' . $roleTitle . ' baru: ' . $nama,
            'tabel_terkait' => 'admin',
            'id_data' => $adminId,
        ];
        ...
        echo json_encode(['success' => true, 'message' => $roleTitle . ' berhasil ditambahkan']);
    } else {
        echo json_encode([
            'success' => false,
            'message' => 'Akun Auth berhasil, tapi gagal menyimpan data admin'
        ]);
    }
    exit;
}
...
<body>
    ...
    <div class="main-wrap">
        <div class="topbar">
            <div class="topbar-inner">
                <h1 class="topbar-title">Admin – Kelola Akun</h1>
            ...
            <div class="form-container">
                <div class="form-section">
                    <div class="inside-header">
                        <h2>Tambah <?= htmlspecialchars($roleTitle) ?></h2>
                    </div>
                    <form id="adminForm" enctype="multipart/form-data">
                        <div class="row-2cols">
                            <div class="form-group">
                                <label class="form-label">Nama Lengkap <span class="text-danger">*</span></label>
                                <input type="text" class="form-control-custom" id="nama_admin" placeholder="Masukkan nama lengkap" required>
                            </div>
                            <div class="form-group">
                        ...
                        <div class="form-actions">
                            <button type="button" class="btn-cancel" onclick="window.location.href='kelola_akun.php?tab=<?= $role ?>'">Batal</button>
    ...
    <script>
        ...
        document.getElementById('adminForm').onsubmit = async function(e) {
            ...
            const nama = document.getElementById('nama_admin').value;
            ...
            try {
                ...
                if (data.success) {
                    setTimeout(() => {
                        window.location.href = 'kelola_akun.php?tab=<?= $role ?>';
                    }, 2000);
                } else {
