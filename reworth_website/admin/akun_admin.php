<?php
    ...
    function getAdminByRole($supabaseUrl, $supabaseKey, $roleName) {
        $url = $supabaseUrl . "/rest/v1/admin?select=*,role!inner(nama_role)&role.nama_role=eq." . urlencode($roleName) . "&order=nama_admin.asc";
        $headers = [
            "apikey: $supabaseKey",
    ...
    $allAdmin = getAdminByRole($supabaseUrl, $supabaseKey, 'admin');
    $total = count($allAdmin);
    $start = ($page - 1) * $per_page;
    $paginatedData = array_slice($allAdmin, $start, $per_page);
        ...
        <table class="responsive-table" id="dynamicTable">
            ...
            <tbody>
                <?php if (!empty($paginatedData)): ?>
                    <?php foreach ($paginatedData as $idx => $a): ?>
                    <tr data-id="<?= $a['id_admin'] ?>">
                        <td><?= $start_number + $idx ?></td>
                        <td>
                            <?php if (!empty($a['foto_profil'])): ?>
                                <img src="<?= getSupabaseImageUrl($a['foto_profil']) ?>" class="avatar-img" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
                            <?php else: ?>
                                <div class="avatar" ><?= strtoupper(substr($a['nama_admin'] ?? 'A', 0, 1)) ?></div>
                            <?php endif; ?>
                        </td>
                        ...
                        <td>
                            <div class="d-flex gap-2 flex-wrap">
                                <button class="btn-aksi btn-edit" onclick="editAkun('admin', '<?= $a['id_admin'] ?>')">
                                    <i class="bi bi-pencil-square"></i> Edit
                                </button>
                                <button class="btn-aksi btn-hapus" onclick="hapusAkun('admin', '<?= $a['id_admin'] ?>')">
