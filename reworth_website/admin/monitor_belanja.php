<?php
    ...
    try {
        $url = $supabaseUrl . "/rest/v1/pesanan?select=*,pengguna!id_pengguna(nama_lengkap,email,foto_profil,no_telepon),produk!id_produk(nama_produk,deskripsi_produk,stok,harga,penjual!id_penjual(nama_penjual))&order=" . $orderBy . "&limit=$limit&offset=$offset";
        ...
        $countUrl = $supabaseUrl . "/rest/v1/pesanan?select=id_pesanan";
        if (!empty($status)) {
        ...
        <table class="responsive-table" id="dynamicTable">
            ...
            <tbody>
                <?php if (empty($pesananList)): ?>
                    <tr>
                        < colspan="6" class="text-center py-5" style="color: #6B8A7E;">
                            <i class="bi bi-cart" style="font-size: 48px; display: block; margin-bottom: 12px; opacity: 0.5;"></i>
                ...
                <?php else: ?>
                    ...
                    foreach ($pesananList as $pesanan): 
                        ...
                        $emailPembeli = '';
                        $teleponPembeli = '';
                        if (isset($pesanan['pengguna']) && is_array($pesanan['pengguna'])) {
                            $namaPembeli = $pesanan['pengguna']['nama_lengkap'] ?? '-';
                            $emailPembeli = $pesanan['pengguna']['email'] ?? '';
                            $teleponPembeli = $pesanan['pengguna']['no_telepon'] ?? '';
                        }
                        
                        $namaProduk = '-';
                        $namaPenjual = '';
                        if (isset($pesanan['produk']) && is_array($pesanan['produk'])) {
                            $namaProduk = $pesanan['produk']['nama_produk'] ?? '-';
                            if (isset($pesanan['produk']['penjual']) && is_array($pesanan['produk']['penjual'])) {
                                $namaPenjual = $pesanan['produk']['penjual']['nama_penjual'] ?? '';
                            }
                        }
                        ...
                        $alamatPengiriman = $pesanan['alamat_pengiriman'] ?? '';
                        $buktiPembayaran = $pesanan['bukti_pembayaran'] ?? '';
                        $jasaKirim = $pesanan['jasa_kirim'] ?? '';
                        $nomorResi = $pesanan['nomor_resi'] ?? '';
                        $alasanPenolakan = $pesanan['alasan_penolakan'] ?? '';
                        $createdAt = $pesanan['created_at'] ?? null;
                    ?>
                        <tr>
                            <td><?= $no++ ?></td>
                            <td>
                                <div class="pembeli-info">
                                    <?= htmlspecialchars($namaPembeli) ?>
                                </div>
                            </td>
                            <td>
                                <div class="produk-info">
                                    <?= htmlspecialchars($namaProduk) ?> <br>
                                    <?php if (!empty($namaPenjual)): ?>
                                        <small class="produk-penjual">
                                            <i class="bi bi-shop"></i> <?= htmlspecialchars($namaPenjual) ?>
                                        </small>
                                    <?php endif; ?>
                                </div>
                            </td>
                            ...
                            <td>
                                <div class="d-flex gap-2 flex-wrap">
                                    <button class="btn-aksi btn-lihat" onclick="lihatDetail('<?= $idPesanan ?>', 'belanja')">
