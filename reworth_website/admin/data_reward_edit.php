<?php
    ...
    $idReward = isset($_GET['id']) ? $_GET['id'] : '';

    if (empty($idReward)) {
        echo '<div class="alert alert-danger m-3">ID Reward tidak ditemukan.</div>';
        exit;
    }
    ...
    function getRewardById($supabaseUrl, $supabaseKey, $id) {
        $ch = curl_init($supabaseUrl . "/rest/v1/reward?id_reward=eq.$id&select=*");
    ...
    $reward = getRewardById($supabaseUrl, $supabaseKey, $idReward);

    if (!$reward) {
        echo '<div class="alert alert-danger m-3">Data reward tidak ditemukan.</div>';
        exit;
    }
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($hapus_foto && !empty($reward['foto_reward'])) {

            $deleteUrl = $supabaseUrl . "/storage/v1/object/media/" . $reward['foto_reward'];
            $deleteCh = curl_init();
        ...
        if (isset($_FILES['foto_reward']) && $_FILES['foto_reward']['error'] === UPLOAD_ERR_OK) {
            ...
            if (!empty($reward['foto_reward']) && !$hapus_foto) {
                $deleteUrl = $supabaseUrl . "/storage/v1/object/media/" . $reward['foto_reward'];
                $deleteCh = curl_init();
                curl_setopt($deleteCh, CURLOPT_URL, $deleteUrl);
                curl_setopt($deleteCh, CURLOPT_CUSTOMREQUEST, "DELETE");
            ...
            if ($httpCode == 200 || $httpCode == 201) {
                $data['foto_reward'] = $filename;
            }
        }
        
        $url = $supabaseUrl . "/rest/v1/reward?id_reward=eq." . $idReward;
        ...
        if ($httpCode === 200 || $httpCode === 204) {
            ...
            $logData = [
                ...
                'aktivitas' => 'Mengedit data reward: ' . $nama_reward,
            ...
            echo json_encode(['success' => true, 'message' => 'Reward berhasil diupdate']);
        } else {
            echo json_encode(['success' => false, 'message' => 'Gagal mengupdate data reward']);
        }
        exit;
    }
?>
...
<head>
    ...
    <title>Edit Reward - Kelola Data Master</title>
            ...
            <div class="form-container">
                <div class="form-section">
                    <div class="inside-header">
                        <h2>Edit Data Reward</h2>
                    </div>
                    <form id="rewardForm" enctype="multipart/form-data">
                        <div class="row-2cols">
                            <div class="form-group">
                                <label class="form-label">Nama Reward <span class="text-danger">*</span></label>
                                <input type="text" class="form-control-custom" id="nama_reward" value="<?= htmlspecialchars($reward['nama_reward'] ?? '') ?>" required>
                                <small class="text-muted" style="font-size: 11px;">Contoh: Voucher Pulsa 10k, Listrik 20k</small>
                            </div>
                            <div class="form-group">
                                <label class="form-label">Kode Voucher</label>
                                <input type="text" class="form-control-custom" id="kode_voucher" value="<?= htmlspecialchars($reward['kode_voucher'] ?? '') ?>">
                        ...
                        <div class="row-2cols">
                            <div class="form-group">
                                ...
                                <div style="position: relative;">
                                    ...
                                    <input type="number" class="form-control-custom" id="poin_dibutuhkan" value="<?= $reward['poin_dibutuhkan'] ?? 0 ?>" style="padding-left: 48px;" required>
                                </div>
                            ...
                            <div class="form-group">
                                ...
                                <input type="number" class="form-control-custom" id="stok" value="<?= $reward['stok'] ?? 0 ?>" required>
                        ...
                        <div class="form-group">
                            ...
                            <div class="file-upload-wrapper">
                                <label class="file-upload-label">
                                    ...
                                    <span id="fileName"><?= !empty($reward['foto_reward']) ? 'Ganti foto reward' : 'Pilih file foto reward' ?></span>
                            ...
                            <div class="image-preview" id="imagePreview">
                                <?php if (!empty($reward['foto_reward'])): ?>
                                    <div class="preview-item">
                                        <img src="<?= htmlspecialchars(getSupabaseImageUrl($reward['foto_reward'])) ?>" alt="Preview">
                        ...
                        <div class="form-actions">
                            ...
                            <button type="submit" class="btn-submit">Simpan Data Perubahan</button>
                        </div>
                    </form>
                </div>
            </div>
    ...
    <script>
        function previewImage(input) {
            ...
            if (input.files && input.files[0]) {
                if (input.files[0].size > 2 * 1024 * 1024) {
                    ...
                    input.value = '';
                    document.getElementById('fileName').textContent = '<?= !empty($reward['foto_reward']) ? 'Ganti foto reward' : 'Pilih file foto reward' ?>';
                    return;
                }
        ...
        function removeExistingImage() {
            ...
            if (!hiddenInput) {
                ...
                hiddenInput.value = '1';
                document.getElementById('rewardForm').appendChild(hiddenInput);
            } else {
                hiddenInput.value = '1';
            }
        }
