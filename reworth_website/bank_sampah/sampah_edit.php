...
$rows = sbGet($supabaseUrl, $supabaseKey, "/rest/v1/jenis_sampah?id_jenis=eq.".urlencode($id)."&select=*&limit=1");
        ...
        <div class="form-card form-card-padded">
            ...
            <div class="fields-wrap" style="padding:0;">
                ...
                <div class="info-badge">
                    ...
                    Ditambahkan: <?=date('d M Y', strtotime($s['created_at']))?>
                </div>
                <?php endif;?>
                ...
                <div class="row-2">
                    <div>
                        ...
                        <input id="namaSampah" type="text" class="field-ul"
                               value="<?=htmlspecialchars($s['nama_sampah']??'')?>"
                    ...
                    <div>
                        ...
                        <input id="hargaPerKg" type="number" class="field-ul"
                               value="<?=(float)($s['harga_per_kg']??0)?>"
                               placeholder="Harga per kg" min="0"
                               oninput="previewHarga()">
...
async function simpanData(){
    ...
    if(res.ok){
        ...
        if (ADMIN_ID) fetch(SB_URL + '/rest/v1/log_admin', { method: 'POST', headers: { 'apikey': SB_KEY, 'Authorization': 'Bearer ' + SB_KEY, 'Content-Type': 'application/json' }, body: JSON.stringify({ id_admin: ADMIN_ID, aktivitas: "Mengedit data jenis sampah", tabel_terkait: 'jenis_sampah', id_data: JENIS_ID, created_at: new Date().toISOString().split('.')[0] + 'Z' }) });
