...
<title>Tambah Jadwal Ambil Sampah</title>
            ...
            <div class="fields-wrap" style="padding:0;">
                ...
                <div class="row-2">
                    <div>
                        ...
                        <input type="time" id="waktuMulai" class="field-ul" value="08:00">
                    ...
                    <div>
                        ...
                        <input type="time" id="waktuSelesai" class="field-ul" value="10:00">
...
function validate(){
    ...
    setErr('tanggal',    'errTanggal', !!tgl);
    setErr('waktuMulai', 'errMulai',   !!mul);
    setErr('waktuSelesai','errSelesai', !!sel);
    setErr('kuota',      'errKuota',   kuo>=1);
    if(ok && mul >= sel){
...
async function simpanData(){
    ...
    try{
        const res=await fetch(`${SB_URL}/rest/v1/jadwal_ambil`,{
            method:'POST',
            headers:{
                'apikey':SB_KEY,
        ...
        const NEW_ID = inserted[0]?.id_jadwal || null;
        const ADMIN_ID = "<?= $_SESSION['id_admin'] ?? '' ?>";
        if (ADMIN_ID) fetch(SB_URL + '/rest/v1/log_admin', { method: 'POST', headers: { 'apikey': SB_KEY, 'Authorization': 'Bearer ' + SB_KEY, 'Content-Type': 'application/json' }, body: JSON.stringify({ id_admin: ADMIN_ID, aktivitas: "Menambahkan jadwal ambil sampah", tabel_terkait: 'jadwal_ambil', id_data: NEW_ID, created_at: new Date().toISOString().split('.')[0] + 'Z' }) });
    
            showToast('Jadwal berhasil ditambahkan!','success');
            setTimeout(()=>window.location.href='jadwal_ambil_sampah.php',900);
        } else {
            const e=await res.json();
            showToast(e.message||'Gagal menyimpan jadwal.','error');
            btn.disabled=false;
