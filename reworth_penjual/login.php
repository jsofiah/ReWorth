    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if (!empty($result)) {
            ...
            if ($penjual['password'] == $password) {
                ...
                if (($penjual['status'] ?? '') !== 'verified') {

                    $error = "Akun Anda belum diverifikasi oleh admin. Mohon ditunggu!";

                } else {
                    $_SESSION['id_penjual'] = $penjual['id_penjual'];
                    $_SESSION['nama_penjual'] = $penjual['nama_penjual'];
                    $_SESSION['email'] = $penjual['email'];
                    $_SESSION['foto_profil'] = $penjual['foto_profil'];
...
<head>
    ...
    <style>
        *, *::before, *::after {
        ...
        .panel-left {
            flex: 1;
            background: url(img/background_login.jpg) no-repeat center/cover;
            display: flex;
