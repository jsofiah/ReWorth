...
<body>
    ...
    <div class="main-wrap">
        ...
        <div class="action-bar-wrap">
            <div class="action-bar">
                <select id="filterStatus" class="form-select w-auto" onchange="window.location.href='?filter='+this.value">
                ...
                <a href="langganan.php" class="btn btn-outline-secondary btn-sm">
            ...
            <div class="card-custom">
                <div class="table-scroll-wrapper">
                    <table class="responsive-table">
                        <thead>
                            <tr><th>No</th><th>Periode</th><th>Jumlah Bayar</th><th>Status</th><th>Aksi</th></tr>
                        </thead>
                        <tbody>
                            <?php if (empty($current_data)): ?>
                                <tr><td colspan="5" class="text-center py-4">Belum ada riwayat langganan</td></tr>
                            <?php else: ?>
                ...
                <div class="table-footer">
                    ...
                    <div class="pagination-custom">
                        <?php if ($current_page > 1): ?>
                            <a href="?page=<?= $current_page-1 ?>&filter=<?= $filterStatus ?>" class="page-btn page-btn-text">Prev</a>
                        <?php endif; ?>
                        <?php for ($i = 1; $i <= $total_pages; $i++): ?>
                            <a href="?page=<?= $i ?>&filter=<?= $filterStatus ?>" class="page-btn <?= $i == $current_page ? 'active' : '' ?>"><?= $i ?></a>
                        <?php endfor; ?>
                        <?php if ($current_page < $total_pages): ?>
                            <a href="?page=<?= $current_page+1 ?>&filter=<?= $filterStatus ?>" class="page-btn page-btn-text">Next</a>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        function lihatDetail(id) {
            fetch('langganan_riwayat_detail.php?id=' + id)
                .then(res => res.text())
                .then(html => {
